...
class ZEnhanceDataset(Dataset):
    ...
    def __init__(self, data_root, data_len=None, mask_config={}, image_size=256, loader=tif_loader, mode='train',
                 trd=None):
        self.path = data_root
    ...
    def __getitem__(self, index):
        ...
        # if self.mode == 'train':  # THIS WAS FOR USING PSEUDO
        ...
        if self.mode == 'train':
            if self.mask_type == 'downsample':
                cond_image = self.generate_yz_like(img)  # ! USE DOWNSAMPLE
            elif self.mask_type == 'gan':
                cond_image = self.loader('{}/{}'.format(self.path[1], file_name), self.trd)  # USE GAN
        else:
        ...
        # mask = self.get_mask()
        # mask_img = t_img*(1. - mask)# + mask*torch.randn_like(t_img)   # WE HAVE NOT USE MASK FOR A WHILE
    ...
    def get_mask(self):  # AGAIN THIS WAS FOR MASKING
        h = w = self.image_size
